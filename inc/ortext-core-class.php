...
class OrTextBase
{
    ...
    const PATCH_PLUGIN = 'bd-original-texts-yandex'; //Директория плагина
...
//				add_action( 'future_' . $v, array( $this, 'futureSentYandex' ), 10, 2 ); //Публикация отложенной записи
        ...
        add_action('transition_post_status', array($this, 'status_change_watcher'), 5, 3);
    ...
    //CODE COMPLETION
    public function status_change_watcher($new_status, $old_status, $post)
    {
        if (in_array($post->post_type, get_option('ortext_posttype', []))) {
            if ($new_status != $old_status && $new_status == 'publish') {
                $this->futureSentYandex($post->ID, $post);
            }
        }
    }
        ...
        if (true == false) {
            if (get_current_screen()->parent_base != 'edit') { //Определяет текущую страницу пользователя
                ...
                <script type='text/javascript'
                        src="<?php echo plugins_url() . '/' . self::PATCH_PLUGIN . '/' . 'js/admin_order.js'; ?>"></script>
                <?php
            }
        }
    ...
    {
        $page_option = add_options_page(self::NAME_TITLE_PLUGIN_PAGE, self::NAME_MENU_OPTIONS_PAGE, 'activate_plugins', self::URL_ADMIN_MENU_PLUGIN, array(
            $this,
            'showSettingPage'
        ));
...
// Убедимся что поле установлено.
    ...
    public function futureSentYandex($post_id, $post)
    {
        ...
        $postData = get_post($post_id);
        do_action('shortcodes_render_out');
        $textNostrip = do_shortcode($postData->post_content);
        $textNostrip = strip_tags($textNostrip);
        $text = htmlspecialchars($textNostrip);
        $text = strip_shortcodes($text);

        wp_mail('petrozavodsky@gmail.com','Origina text', $text);
